...
const UploadImages = ({updateContentImages, allContentImages, displayAlert}) => {
    ...
    const handleDeleteFile = async (url) =>{
        if(url !== ''){
            ...
            deleteObject(fileRef).then(() => {
              displayAlert(true, 'Deleted Successfully')
    ...
    const storageRef = ref(storage, `postcontentimages/${userPhoto.name}-${new Date().toUTCString()}`)
  ...
  return (
    <Box width={'100%'}>
    <ComponentTitle title={'Upload Post Related Images'}/>
       <Box display={'flex'} justifyContent='center' width={'100%'} marginBottom='20px'>
          ...
          {allContentImages.length ?  <p>Drag and Drop Images</p> : null}
          <Box display={'flex'} flexDirection='row' justifyContent={'center'} marginTop='10px'>
         
            {allContentImages.length ? allContentImages.map((image) => {
              return   (
                        <Box>
                          <img src={image} width='50px' />
                          <IconButton onClick={()=> handleDeleteFile(image)}>
                          <CloseIcon/>
                          </IconButton>
                       </Box>
                       )
